def get_put_away_plates_goal(sim, fn_prototype=False):
    goal = 'plate'
    ...
    return goals, f"Put {plates_str} and {plates[-1]['class_name']}_{plates[-1]['id']} into the {cabinet_node['class_name']}_{cabinet_node['id']}"

def get_throw_away_pepper_goal(sim, fn_prototype=False):
    graph = sim.get_graph()
...
def get_wash_then_put_away_plates_goal(sim, fn_prototype=False):
    goal = 'plate'
    goal_start = "kitchentable"
    goal_location1 = 'kitchencabinet'
    goal_wash_loc = "sink"
...
def get_put_salmon_in_fridge_goal(sim, fn_prototype=False):
    graph = sim.get_graph()
...
def get_cook_salmon_in_microwave_goal(sim, fn_prototype=False):
    graph = sim.get_graph()
    salmon = [node for node in graph["nodes"] if node["class_name"] == "salmon"][0]
...
def get_make_toast_goal(sim, fn_prototype=False):
    graph = sim.get_graph()

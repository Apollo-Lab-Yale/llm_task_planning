...
from llm_task_planning.planner.prog_prompt.utils_aug_env import get_obj_ids_for_adding_states, add_additional_obj_states
...
def run_execution(args, comm, test_tasks, gen_plan, log_file):
        ...
        for subgoal in subgoals.keys():
            ...
            for action in subgoals[subgoal]:
                ...
                if len(action)==3 and "put" in action[0]: # 2 objs action
                    ...
                    all_failures.append(f"Action {action} not executable.")
                    sim_actions.append("")
                ...
                if not success:
                    ...
                    all_failures.append(f"Sim execution failed: {executor.info.get_error_string()}")
                    step+=1
                else:
                    all_failures.append("")
    ...
    return sim_actions, all_failures, exec_per_task
